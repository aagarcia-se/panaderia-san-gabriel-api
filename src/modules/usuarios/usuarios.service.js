...
import { actualizarUsuarioDao, consultarUsuariosDao, crearUsuarioDao, bloquearUsuarioDao, elminarUsuarioDao, desbloquearUsuarioDao, cambiarPasswordDao, actualizarDatosUsuarioDao, ingresarContraseniaTemporalDao, actualizarContraseniaDao } from "./usuarios.dao.js";
...
export const actualizarDatosUsuarioService = async (dataUsuario) => {
  try {
    const result = await actualizarDatosUsuarioDao(dataUsuario);
    if (result === 0) {
      const error = getError(3);
      throw new CustomError(error);
    }
...
export const resetearContraseniaServices = async (idUsuario) => {
  try {
    ...
    const result = await ingresarContraseniaTemporalDao(idUsuario, hashedPass);
    if (result === 0) {
      const error = getError(3);
      throw new CustomError(error);
    }
...
export const actualizarContraseniaServices = async (data) => {
  try {
    const result = await actualizarContraseniaDao(data);
    if (result === 0) {
      const error = getError(3);
      throw new CustomError(error);
    }
